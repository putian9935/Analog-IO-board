...
// IIRcontroller in its polynomial form 
...
struct IIRCascadeController : public Controller {
    ...
    IIRCascadeController(read_func_t reader, write_func_t writer, double const (&zeroes)[len_zeroes], double const (&poles)[len_poles], double const overall_gain, double const lower, double const upper) : Controller(reader, writer), overall_gain(overall_gain), lower(lower), upper(upper)
    {
        int i = 0; 
        for (;i < len_zeroes; ++i)
